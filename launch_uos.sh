...
# NVME pass-through
modprobe pci_stub
echo ${passthru_vpid["uart"]} > /sys/bus/pci/drivers/pci-stub/new_id
echo ${passthru_bdf["uart"]} > /sys/bus/pci/devices/${passthru_bdf["uart"]}/driver/unbind
echo ${passthru_bdf["uart"]} > /sys/bus/pci/drivers/pci-stub/bind
acrn-dm -A -m $mem_size -s 0:0,hostbridge \
  -U d2795438-25d6-11e8-864e-cb7a18b34643 \
  -s 2,pci-gvt -G "$2" \
  -s 8,passthru,00/16/3 \
...
if [ "$1" = "-C" ];then
	...
	launch_clear 1 "64 448 8" 0x000F00
fi
